---
name: course-review
description: 基于9大学习科学理论的通用课程交互式复习方案生成器v4.2——三层深度选择+自适应学习路径+间隔重复+错因分类+正反馈系统+纸质输出全场景+知识点屏蔽+AI出题助手+首次欢迎引导+考试倒计时+免责声明+截图预览模式。面向基础薄弱、短时间需完成复习的群体，帮助AI为任意课程创建定制化单文件HTML复习应用。适用于大学课程考前速通、职业资格考试冲刺、跨专业补课等场景。
---

# Course Review Skill v4.2 — 学习科学驱动的课程速通复习应用生成器

> **核心理念**：不是文档，是学习应用。融合费曼学习法、主动回忆、间隔重复、掌握学习、最近发展区、认知负荷理论、元认知训练、自我效能感、情境认知9大学习科学理论，用精准急救设计哲学帮基础薄弱学生在最短时间内建立应试框架。v4.2在v4.0基础上新增**首次欢迎引导浮层**（帮助新用户快速上手）、**考试倒计时**（设置考试日期后自动显示，颜色随紧迫感变化）、**COURSE_META个性化配置**（学校/姓名/考试日期）、**标准化免责声明**（明确AI辅助生成性质）、**截图预览模式**（URL加?preview=1一键展开全部内容方便截图/打印），并清理了template.html头部开发者注释。
...
v4.2 在v4.0基础上新增5项体验增强功能：

| 新系统/组件 | 学习科学/体验价值 | 核心价值 | 版本 |
|--------|-------------|---------|------|
| 首次欢迎引导浮层 | 降低认知负荷（新手引导） | 第一次打开时三档说明+开始按钮，降低上手门槛 | v4.2 |
| 考试倒计时模块 | 自我效能感（紧迫感驱动） | Header显示倒计时，颜色随天数绿→黄→红变化 | v4.2 |
| COURSE_META个性化配置 | 情境认知 | 支持学校/专业/姓名/考试日期配置，定制专属感 | v4.2 |
| 标准化免责声明 | 透明度 | 明确AI辅助生成性质，建议结合课件综合复习 | v4.2 |
| 截图预览模式 | 全场景适配 | URL加`?preview=1`一键展开所有手风琴，方便截图/打印 | v4.2 |
...
#### 4.1 COURSE_META 配置（v4.2必选）

JS开头必须定义COURSE_META对象：
...
var COURSE_META = {
  ...
  major: '',           // 专业（可选）
  studentName: '',     // 姓名（可选）
  examDate: '',        // 考试日期 YYYY-MM-DD（用户提供时填写，用于倒计时）
  version: '1.0'
};
var SLUG = COURSE_META.courseSlug;
```

- `school`非空时：Header显示学校/专业专属徽章
- `studentName`非空时：欢迎浮层显示"为XX同学准备"
- `examDate`非空时：Header显示考试倒计时（绿>7天/黄3-7天/红<3天脉冲/考试当天/已结束）
- 全部为空时：不显示任何个性化信息，保持通用外观

#### 4.2 必须包含的v4.2新组件

1. **首次欢迎浮层**：首次打开时显示（localStorage记忆），三档说明+开始按钮
2. **考试倒计时徽章**：Header中，仅examDate非空时显示，颜色随天数动态变化
3. **标准化免责声明**：footer之前琥珀色警告块，说明AI辅助生成性质
4. **截图预览模式**：URL加`?preview=1`自动展开所有手风琴+截图标识，方便截图打印

#### 4.3 页面结构（宏观流程）

```
[Header: 标题+学校徽章(可选)+三层深度选择器+倒计时(可选)+CTA]
  ↓
...
⚠️ 陷阱库（计算型：计算陷阱库；概念型：常见误区库）
  ↓
🎯 题库刷题（多模式:顺序/随机/章节/间隔复习/错题本+AI出题助手）【v4.0】
  ↓
✅ 考前自测清单（可勾选能力清单+完成度）
  ↓
...
```
course-review-skill/
├── SKILL.md                # TRAE格式（本文件，v4.2学生体验增强版）
├── AGENTS.md               # 跨平台标准格式
├── CLAUDE.md               # Claude Code格式
├── .cursorrules            # Cursor格式
├── .windsurfrules          # Windsurf格式
├── .clinerules/course-review.md  # Cline格式
├── skill.json              # Skill配置v4.2
├── README.md               # 人类可读文档v4.2
├── template.html           # 参考模板v4.2（含14个标准组件+4个Canvas可视化+欢迎浮层+倒计时+免责声明+截图模式）
...
```
请使用 course-review skill v4.2 为【课程名称】创建交互式复习应用。
...
- 复习时间：【1天/2天/3天】
- 目标分数：【补考救命/稳过及格/冲刺高分】
- 学校/专业（可选）：【填写后Header显示专属徽章】
- 姓名（可选）：【填写后欢迎页显示定制语】
- 考试日期（可选）：【YYYY-MM-DD，填写后显示倒计时】
...
14. v4.2必含：COURSE_META配置+首次欢迎浮层+标准化免责声明+考试倒计时（有examDate时）+截图预览模式(?preview=1)
15. 化学课程加载mhchem扩展，物理课程加载physics扩展
```

---

**v4.2的灵魂是9大学习科学理论的系统融合，骨架是自适应学习路径引擎，血肉是例题驱动七段式，校准器是参考材料，体验层是情感设计与正反馈。v4.2在学生体验上的关键提升是：首次欢迎浮层降低上手门槛、考试倒计时提供紧迫感驱动、COURSE_META让每个学生拥有专属感、免责声明保持透明、截图预览方便分享。AI在创建复习方案时，请始终问自己：这符合哪个学习科学原理？基础薄弱的学生看得懂吗？短时间能看完吗？第一次打开知道怎么开始用吗？内容是否在学生的最近发展区内？是否给了足够的正向反馈？学生能感知到自己的进度吗？错题是否转化为了精准的诊断数据？**
